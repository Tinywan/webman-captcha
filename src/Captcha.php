...
class Captcha
{
    ...
    {
        if (!Redis::exists('captcha')) {
            return false;
        }
        $key = Redis::hGet('captcha','key');
    ...
    public function createBase64(string $config = null)
    {
        ...
        return 'data:image/png;base64,' . base64_encode($content);//合成图片的base64编码
    }
    ...
    protected function writeCurve(): void
    {
        ...
        $px1 = 0; // 曲线横坐标起始位置
        $px2 = mt_rand($this->imageW / 2, (int)$this->imageW); // 曲线横坐标结束位置
        ...
        for ($px = $px1; $px <= $px2; $px = $px + 1) {
            if (0 != $w) {
                ...
                while ($i > 0) {
                    imagesetpixel($this->im, $px + $i, $py + $i, (int)$this->color); // 这里(while)循环画像素点比imagettftext和imagestring用字体大小一次画出（不用这while循环）性能要好很多
                    $i--;
                }
            }
        }
        ...
        for ($px = $px1; $px <= $px2; $px = $px + 1) {
            if (0 != $w) {
                ...
                $i  = (int) ($this->fontSize / 5);
                while ($i > 0) {
                    imagesetpixel($this->im, $px + $i, $py + $i, (int)$this->color);
                    $i--;
                }
            }
        }
