...
class Captcha
{
    ...
    protected function writeCurve(): void
    {
        ...
        $px1 = 0; // 曲线横坐标起始位置
        $px2 = mt_rand((int) ($this->imageW / 2), (int)$this->imageW); // 曲线横坐标结束位置
        ...
        for ($px = $px1; $px <= $px2; $px = $px + 1) {
            if (0 != $w) {
                ...
                while ($i > 0) {
                    imagesetpixel($this->im, (int) $px + $i, (int) $py + $i, (int)$this->color); // 这里(while)循环画像素点比imagettftext和imagestring用字体大小一次画出（不用这while循环）性能要好很多
                    $i--;
                }
            }
        }
